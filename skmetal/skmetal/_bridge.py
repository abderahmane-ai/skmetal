...
try:
    ...
    warnings.warn(
        f"skmetal: Metal GPU not available ({_metal_err}). All estimators will run on CPU via scikit-learn.",
        stacklevel=2,
    )
    _lib = None
...
_BRIDGE_REGISTRY = [
    ...
    (
        "skmetal_knn_vote_classify_weighted",
        ctypes.c_void_p,
        ctypes.c_void_p,
        ctypes.c_void_p,
        ctypes.c_void_p,
    ...
    (
        "skmetal_knn_vote_regress_weighted",
        ctypes.c_void_p,
        ctypes.c_void_p,
        ctypes.c_void_p,
        ctypes.c_void_p,
...
def _should_use_f16(A: np.ndarray, B: np.ndarray) -> bool:
    ...
    return (
        M <= _F16_SIMD_MAX_DIM
        and N <= _F16_SIMD_MAX_DIM
        and K <= _F16_SIMD_MAX_DIM
        and M % _F16_SIMD_ALIGN == 0
        and N % _F16_SIMD_ALIGN == 0
        and K % _F16_SIMD_ALIGN == 0
        and M >= _F16_SIMD_MIN_DIM
        and N >= _F16_SIMD_MIN_DIM
        and K >= _F16_SIMD_MIN_DIM
    )
...
def _use_mlx_gemm(A: np.ndarray, B: np.ndarray, trans_A: bool = False, trans_B: bool = False) -> bool:
...
def _gemm_mlx(A: np.ndarray, B: np.ndarray, alpha=1.0, beta=0.0, trans_A=False, trans_B=False) -> np.ndarray:
...
def gemm(A: np.ndarray, B: np.ndarray, alpha=1.0, beta=0.0, trans_A=False, trans_B=False) -> np.ndarray:
    ...
    _bridge_call(
        ...
        ctypes.c_float(beta),
        ctypes.c_int(int(trans_A)),
        ctypes.c_int(int(trans_B)),
    )
    return C
...
def kmeans_assign(X: np.ndarray, centroids: np.ndarray, assignments: np.ndarray, n: int, d: int, k: int) -> None:
...
def kmeans_batch_fused(X, centroids, assignments, n, d, k, num_groups, max_iter, tol):
    ...
    _bridge_call(
        _lib.skmetal_kmeans_batch_fused,
        X,
        centroids,
        assignments,
        n,
...
def compute_mindists(
    X: np.ndarray, centroids: np.ndarray, assignments: np.ndarray, dists: np.ndarray, n: int, d: int, k: int
) -> None:
    """Per-point squared distance to assigned centroid."""
    _bridge_call(_lib.skmetal_compute_mindists, X, centroids, assignments, dists, n, d, k)
...
def kmeans_inertia(X: np.ndarray, centroids: np.ndarray, assignments: np.ndarray, n: int, d: int, k: int) -> float:
    ...
    return _lib.skmetal_kmeans_inertia(X.ctypes.data, centroids.ctypes.data, assignments.ctypes.data, n, d, k)


def ridge_fit_solve(X: np.ndarray, y: np.ndarray, X_mean: np.ndarray, coef: np.ndarray, alpha: float) -> None:
    """Fused Ridge: center X + XTX + XTy + L2 + Cholesky solve (one GPU dispatch)."""
    _bridge_call(_lib.skmetal_ridge_fit_solve, X, y, X_mean, coef, alpha, X.shape[0], X.shape[1])
...
def knn_vote_classify(
    indices: np.ndarray, train_labels: np.ndarray, predictions: np.ndarray, N: int, k: int, n_train: int
) -> None:
    """Majority-vote classification from k-NN indices."""
    _bridge_call(_lib.skmetal_knn_vote_classify, indices, train_labels, predictions, N, k, n_train)


def knn_vote_regress(
    indices: np.ndarray, train_targets: np.ndarray, predictions: np.ndarray, N: int, k: int, n_train: int
) -> None:
    """Mean regression from k-NN indices."""
    _bridge_call(_lib.skmetal_knn_vote_regress, indices, train_targets, predictions, N, k, n_train)


def knn_vote_classify_weighted(
    indices: np.ndarray,
    ...
    _bridge_call(_lib.skmetal_knn_vote_classify_weighted, indices, distances, train_labels, predictions, N, k, n_train)


def knn_vote_regress_weighted(
    indices: np.ndarray,
    distances: np.ndarray,
    ...
    _bridge_call(_lib.skmetal_knn_vote_regress_weighted, indices, distances, train_targets, predictions, N, k, n_train)


def knn_tiled_kneighbors(
    X_query: np.ndarray, X_train: np.ndarray, k: int, tile_size: int = 4096, metric: str = "euclidean"
    ...
    if err != 0:
        raise RuntimeError(
            f"knn_tiled_kneighbors failed with code {err} (n_q={n_q}, n_t={n_t}, d={d}, k={k}, metric={metric})"
        )
...
def minmax_transform(
    ...
    _bridge_call(
        ...
        X.shape[0],
        X.shape[1],
        ctypes.c_float(feature_min),
        ctypes.c_float(feature_max),
    )


def column_transform(input: np.ndarray, output: np.ndarray, center: np.ndarray, scale: np.ndarray) -> None:
    """output[i][j] = (input[i][j] - center[j]) * scale[j]."""
    _bridge_call(_lib.skmetal_column_transform, input, output, center, scale, input.shape[0], input.shape[1])
...
def tree_predict_all(
    X: np.ndarray,
    all_tree_values: np.ndarray,
    all_tree_feature: np.ndarray,
    all_tree_threshold: np.ndarray,
    all_tree_left: np.ndarray,
    all_tree_right: np.ndarray,
    all_tree_is_leaf: np.ndarray,
    tree_offsets: np.ndarray,
    tree_n_nodes: np.ndarray,
    baseline: np.ndarray,
    predictions: np.ndarray,
) -> None:
    """Predict by traversing ALL trees in a single kernel dispatch."""
    _bridge_call(
        _lib.skmetal_tree_predict_all,
        X,
        all_tree_values,
        all_tree_feature,
        all_tree_threshold,
...
def rbf_kernel_square(X: np.ndarray, K_out: np.ndarray, gamma: float) -> None:
    ...
    """
    _bridge_call(_lib.skmetal_rbf_kernel_square, X, K_out, gamma, X.shape[0], X.shape[1])


def rbf_kernel_cross(X1: np.ndarray, X2: np.ndarray, K_out: np.ndarray, gamma: float) -> None:
    """Cross RBF kernel: K[i][j] = exp(-gamma * ||X1[i] - X2[j]||^2).
    Row norms for both matrices computed internally on GPU.
    """
    _bridge_call(_lib.skmetal_rbf_kernel_cross, X1, X2, K_out, gamma, X1.shape[0], X2.shape[0], X1.shape[1])


def svc_predict_binary(
    X_test: np.ndarray,
    X_sv: np.ndarray,
    ...
    """
    _bridge_call(
        _lib.skmetal_svc_predict_binary,
        X_test,
        X_sv,
        dual_coef,
        intercept,
...
def logreg_irls_fit(
    X: np.ndarray, y: np.ndarray, C: float, tol: float, max_iter: int, fit_intercept: bool
) -> tuple[np.ndarray, int]:
    ...
    _bridge_call(
        ...
        ctypes.c_float(tol),
        ctypes.c_int32(max_iter),
        ctypes.c_int32(int(fit_intercept)),
        ctypes.c_size_t(n),
        ctypes.c_size_t(p),
        ctypes.byref(n_iter_out),
    )
...
def multinomial_lbfgs_fit(
    X: np.ndarray, y_enc: np.ndarray, C: float, tol: float, max_iter: int, n_classes: int
) -> tuple[np.ndarray, int]:
    ...
    n_iter_out = ctypes.c_int32(0)
    _bridge_call(
        _lib.skmetal_multinomial_lbfgs_fit,
        X,
        y_enc,
        W,
        ctypes.c_float(C),
